<div>
    <div class="flex justify-between px-4 mx-auto max-w-7xl sm:px-6 md:px-8">
        <h1 class="text-2xl font-semibold text-gray-900">Pending Review</h1>
        <div>
                        ...
                        >
                            ...
                            >
                                <thead class="bg-gray-50">
                                    <tr>
                                        <x-table.th>Virtual ID</x-table.th>
                                ...
                                >
                                    ...
                                    <tr wire:key="member-{{ $user->uuid }}">
                                        <x-table.td>{{ $user->person->virtual_id }}</x-table.td>
                                        <x-table.td>{{ $user->person->getFullName() }}</x-table.td>
                                        <x-table.td>{{ Str::upper($user->person->barangay->name) }}</x-table.td>
                                        <x-table.td>
                                            <x-label-status status="{{ $user->paymentStatus() ? 'success' : 'danger' }}">
                                                PAID
                                            </x-label-status>
                                        </x-table.td>
                                        <x-table.td>
                                            <x-label-status status="{{ $user->labelStatus() }}">
                                                {{ $user->status }}
                                            </x-label-status>
                                        </x-table.td>
                                        <x-table.td>
                                            <x-button.button-link href="{{ url('admin/members/review' , $user->uuid) }}">Review</x-button.button-link>
                                        </x-table.td>
                                    </tr>
                                    @endforeach
                                </tbody>
