<div>
    <div class="flex justify-between px-4 mx-auto max-w-7xl sm:px-6 md:px-8">
        <h1 class="text-2xl font-semibold text-gray-900">Registrants</h1>
        <div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <tr wire:key="member-{{ $user->uuid }}">
                                        ...
                                        <x-table.td>
                                            <span class="uppercase">{{ $user->person->barangay->name }}</span>
                                        </x-table.td>
                                        <x-table.td>{{ $user->person->purok }}</x-table.td>
                                        <x-table.td>
                                            <x-label-status color="{{ $user->labelStatus() }}">
                                                {{ $user->status }}
                                            </x-label-status>
                                        </x-table.td>
                                        <x-table.td>
                                            @role('cashier')
                                            <x-button.button-link  href="{{ url('admin/mortuary/collections/create?template=registration&search_keyword=' . $user->person->virtual_id) }}">
                                                Pay Registration
